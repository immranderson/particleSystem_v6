...
public class RenderClass_v5 extends JPanel implements MouseListener, MouseMotionListener, KeyListener {
  ...
  public void glow(int light, int x, int y) { 
    ...
    if (blockArray[x][y].type == 0) {
      ...
      if (light <= lightCutoff || x <= 0 || x >= (int) (WIDTH / 32) || y <= 0 || y >= (int) (HEIGHT / 32)/*if light <= 1 || location invalid*/) {
        //base case
        //do nothing
      } else {
      ...
      if (light <= lightCutoff || x <= 0 || x >= (int) (WIDTH / 32) || y <= 0 || y >= (int) (HEIGHT / 32)) {
        //base case
        //do nothing
      } else {
    ...
    if (blockArray[x][y].type == 0) {
      for (int x_I = 0; x_I < 8; x_I++) {
        for (int y_I = 0; y_I < 8; y_I++) {
          ...
          blockRaster[x * 8 + x_I + (y * 8 + y_I) * WIDTH / 4] = makeIntColor(255, light, (int) light / 3, 0);
  ...
  public void tick(){
    ...
    for (int x_I = 0, lightWidth = (int) (WIDTH / 32); x_I < lightWidth; x_I++) {  //Draw previous frame's lighting, then clear lightArray
